...
show_info() {
    ...
    PASSWORD=$(cat "$PASS_FILE" 2>/dev/null || echo "未知")
...
EOF
    ...
    restart_service
    echo -e "${GREEN}✅ Hysteria2 Realms 模式安装完成${NC}"
    show_info
}
...
while true; do
    ...
    echo -e "${GREEN}===============================================${NC}"
    echo -e "     Hysteria2 Realms 一键管理脚本"
    echo -e "     系统: $OS    状态: $STATUS"
    echo -e "${GREEN}===============================================${NC}"
    echo -e " ${CYAN}[1]${NC} 安装 Hysteria2 (Realms)"
    echo -e " ${CYAN}[2]${NC} 查看配置 & 节点链接"
    echo -e " ${CYAN}[3]${NC} 重启服务"
    echo -e " ${CYAN}[4]${NC} 修改认证密码"
    echo -e " ${CYAN}[5]${NC} 更新 Hysteria2"
    echo -e " ${CYAN}[6]${NC} 卸载 Hysteria2"
    ...
    case $choice in
        1) install_hy2 ;;
        2) show_info ;;
        3) restart_service && echo -e "${GREEN}✅ 服务已重启${NC}" ;;
        4) change_password ;;
        5) update_hy2 ;;
        6) uninstall_hy2 ;;
        0) exit 0 ;;
